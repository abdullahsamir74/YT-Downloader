...
class App:
    ...
    def _prompt_options(self, is_audio: bool = False) -> Optional[DownloadOpts]:
        ...
        if is_audio:
            ...
            bit_choices = [
                Choice("320", "1. 320 kbps"),
                Choice("192", "2. 192 kbps"),
                Choice("128", "3. 128 kbps"),
            ]
            bit_p = inquirer.select(
                message="Bitrate:", choices=bit_choices, raise_keyboard_interrupt=False
            )
    ...
    def _execute_download(
        self, url: str, opts: DownloadOpts, title: str, prefix: Optional[str] = None
    ):
        with Progress(
            SpinnerColumn(),
            ...
            if ok:
                progress.update(
                    task, completed=100, description="[bold green]✓ Completed"
                )
                console.print(
                    f"[bold green]✓ Saved to {opts.output_dir}[/bold green]\n"
                )
            else:
    ...
    def handle_playlist(self):
        ...
        choices = [
            Choice(
                value=item,
                name=f"[{item.index:02d}] {item.title[:50]} ({item.duration_str})",
                enabled=True,
            )
            for item in info.entries
        ]
        selected = inquirer.checkbox(
            message="Select videos (Space toggle, Enter confirm):",
            choices=choices,
            raise_keyboard_interrupt=False,
        ).execute()
        if not selected:
        ...
        for idx, item in enumerate(selected, 1):
            ...
            console.print(
                f"[bold cyan][{idx}/{len(selected)}][/bold cyan] {item.title}"
            )
    ...
    def handle_audio(self):
        ...
        if isinstance(info, PlaylistInfo):
            display_info_card(
                "📚 Playlist Info",
                [("Title:", info.title), ("Total:", len(info.entries))],
            )
            choices = [
                Choice(
                    value=item,
                    name=f"[{item.index:02d}] {item.title[:50]}",
                    enabled=True,
                )
                for item in info.entries
            ]
            ...
            display_info_card(
                "🎵 Video Info",
                [("Title:", info.title), ("Duration:", info.duration_str)],
            )
            opts = self._prompt_options(is_audio=True)
            if not opts:
                return
    ...
    def handle_batch(self):
        ...
        for item in items:
            ...
            st = (
                ...
                else "[bold red]✗ Missing Sub[/bold red]"
            )
        ...
        confirm = inquirer.confirm(
            message=f"Embed subtitles into {len(ready)} video(s)?",
            default=True,
            raise_keyboard_interrupt=False,
        ).execute()
        if confirm:
            ...
            console.print(
                Panel(
                    "[bold green]✓ Subtitles Embedded![/bold green]",
                    border_style="green",
                )
            )
    ...
    def handle_library(self):
        ...
        table = Table(
            title=f"📁 Downloads ({self.output_dir.resolve()})", border_style="green"
        )
        table.add_column("Filename", style="bold white")
        table.add_column("Size", style="cyan")

        for f in sorted(
            self.output_dir.iterdir(), key=lambda x: x.stat().st_mtime, reverse=True
        ):
